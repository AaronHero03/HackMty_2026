...
def cpp(y):
    ...
    Mismo cálculo que la versión trama por trama, pero procesa las tramas en bloques con numpy
    (FFT por fila y un solo polyfit con varias columnas). En llamadas largas pasa de >10 s a ~1 s.
    """
    ...
    mitad = frame_len // 2                      # len(cepstrum) // 2: irfft devuelve frame_len muestras
    q_hi_c = min(q_hi, mitad)
    n_tramas = len(range(0, len(y) - frame_len, hop))
    if n_tramas == 0 or q_hi_c <= q_lo:
        return np.nan

    ventana = np.hanning(frame_len)
    x = np.arange(mitad)
    tramas = np.lib.stride_tricks.sliding_window_view(y, frame_len)[: len(y) - frame_len : hop]

    valores = []
    for i in range(0, n_tramas, 4096):           # por bloques, para no ocupar cientos de MB
        bloque = tramas[i : i + 4096] * ventana
        espectro_log = np.log(np.abs(np.fft.rfft(bloque, axis=1)) + 1e-10)
        cepstrum = np.fft.irfft(espectro_log, axis=1)
        region = cepstrum[:, q_lo:q_hi_c]
        coef = np.polyfit(x, cepstrum[:, :mitad].T, 1)
        base = np.polyval(coef, q_lo + np.argmax(region, axis=1))
        valores.append(np.max(region, axis=1) - base)
    return float(np.mean(np.concatenate(valores)))
